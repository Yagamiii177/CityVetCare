...
const AdoptionRequestDetailScreen = () => {
  ...
  useEffect(() => {
    ...
    const loadStray = async () => {
      ...
      try {
        ...
        // Best-effort: details can still render without images
  ...
  // Parse applicant details if it's a JSON string
  let applicantDetails = {};
  if (request.applicant_details) {
    try {
      applicantDetails =
        typeof request.applicant_details === "string"
          ? JSON.parse(request.applicant_details)
          : request.applicant_details;
    } catch (e) {
      console.error("Failed to parse applicant details", e);
    }
  }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoGroup}>
            <InfoRow label="Request ID" value={`#${request.adoption_id}`} />
            <InfoRow
              label="Submitted"
            ...
            {request.approved_date && (
              <InfoRow
                label="Processed"
                value={new Date(request.approved_date).toLocaleDateString()}
              />
            )}
          </View>
        </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.infoGroup}>
              {applicantDetails.fullName && (
                <InfoRow label="Full Name" value={applicantDetails.fullName} />
              )}
              {applicantDetails.age && (
                <InfoRow label="Age" value={applicantDetails.age} />
              )}
              {applicantDetails.phone && (
                <InfoRow label="Phone" value={applicantDetails.phone} />
              )}
              {applicantDetails.email && (
                <InfoRow label="Email" value={applicantDetails.email} />
              )}
              {applicantDetails.address && (
                <InfoRow label="Address" value={applicantDetails.address} />
              )}
              {applicantDetails.residenceType && (
                <InfoRow
                  label="Residence Type"
                  value={applicantDetails.residenceType}
                />
              )}
              {applicantDetails.adoptionReason && (
                <InfoRow
                  label="Adoption Reason"
                  value={applicantDetails.adoptionReason}
                  multiline
                />
              )}
            </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.helpText}>
            If you have questions about your adoption request, please contact
...
export default AdoptionRequestDetailScreen;
