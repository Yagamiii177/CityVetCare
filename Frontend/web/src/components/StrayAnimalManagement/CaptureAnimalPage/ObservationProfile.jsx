...
const ObservationProfile = ({ animal, onClose, isOpen, onSave }) => {
  ...
  const addNewObservation = () => {
    ...
    setEditedAnimal((prev) => ({
      ...prev,
      pastObservations: [newObservation, ...prev.pastObservations],
    }));
  };
                ...
                <div className="space-y-2">
                  ...
                  <p>
                    ...
                    {isEditing ? (
                      <select
                        name="status"
                        value={editedAnimal.status || "Observation"}
                        onChange={handleInputChange}
                        className="ml-2 border rounded px-2 py-1"
                      >
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs ml-2 ${
                          editedAnimal.status === "Healthy"
                            ? "bg-green-100 text-green-800"
                            : editedAnimal.status === "Injured"
                            ? "bg-yellow-100 text-yellow-800"
                            : "bg-red-100 text-red-800"
                        }`}
                      >
                        {editedAnimal.status || "Observation"}
                      </span>
                    )}
                  </p>
                </div>
              ...
              <div className="space-y-3">
                {sortedObservations.length > 0 ? (
                  sortedObservations.map((obs, index) => (
                    <div
                      key={index}
                      className="border-l-4 border-[#FA8630] pl-4 py-2 relative"
                    >
                      {isEditing && (
                        <button
                      ...
                      {isEditing ? (
                        ...
                        <>
                          <p className="font-medium">{formatDate(obs.date)}</p>
                  ...
                  <p className="text-gray-500">No past observations recorded</p>
                )}
              </div>
            </div>
          </div>
