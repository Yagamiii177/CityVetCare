...
const normalizeAnnouncement = (raw = {}) => {
  ...
  const attachments = (pickField(raw, ["attachments", "files"]) || []).map(
    (file) => ({
      ...
      fileUrl: file.fileUrl || file.file_url || file.url || file.path || "",
    })
  );
...
const buildFormData = (payload = {}) => {
  ...
  if (Array.isArray(payload.attachments)) {
    ...
    console.log("📎 Number of attachments:", payload.attachments.length);
    ...
    console.log(
      "⚠️ No attachments array or not an array:",
      payload.attachments
    );
  }
